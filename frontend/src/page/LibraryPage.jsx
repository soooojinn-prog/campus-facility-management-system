...
// ── 상수 ──
const NAV_TABS = [{key: 'reading', label: '열람실 현황'}, {key: 'books', label: '도서 검색'}, {
  key: 'study', label: '스터디룸 예약',
}, {key: 'congestion', label: '혼잡도'}, {key: 'notices', label: '공지사항'}];
...
// ── 공통 컴포넌트 ──
function LoadingBox() {
  return (<div style={{padding: '40px', color: '#A0AEC0', display: 'flex', alignItems: 'center', gap: 12}}>
    <div style={{
      width: 20,
      height: 20,
...
// 도서 행 공통 컴포넌트
function BookRow({b}) {
  return (<div className="room-row" style={{display: 'flex', alignItems: 'center', gap: 14}}>
    <div style={{
      width: 34,
      height: 44,
      flexShrink: 0,
    ...
    <div style={{flex: 1}}>
      ...
      <div style={{
        fontSize: '.78rem', color: '#718096', marginTop: 2,
    ...
    <div className="floor-detail">
      ...
      {selected && !seatLoading && seatData && (() => {
        ...
        const seats = (seatData.seats || []).map(s => ({
          ...s, status: localReserved[s.seatNo] ? 'RESERVED' : s.status,
        }));
          ...
          <div className="room-category">
            ...
            <div style={{display: 'flex', gap: 14, marginBottom: 12}}>
              {[['AVAILABLE', '이용 가능', '#38A169'], ['OCCUPIED', '사용 중', '#E53E3E'], ['RESERVED', '예약됨', '#D69E2E']].map(([k, v, c]) => (
                  <div key={k} style={{
                    display: 'flex', alignItems: 'center', gap: 5, fontSize: '.73rem', color: '#718096',
                  }}>
      ...
      {selected && !seatLoading && !seatData && (<div style={{padding: 40, color: '#A0AEC0'}}>좌석 정보를 불러올 수 없습니다.</div>)}
    </div>
  </div>);
}
...
function BooksSection({onNeedLogin}) {
  ...
  const quickCards = [{
    key: 'new', icon: '📦', title: '신규 입수', desc: '전체 도서', onClick: handleQuickNew,
  }, {
    key: 'top5', icon: '🔥', title: '인기 도서', desc: 'Top 5', onClick: handleQuickTop5,
  }, {
    key: 'return', icon: '📅', title: '반납 예정', desc: '내 반납', onClick: handleQuickReturn,
  }];

  return (<div className="floor-detail" style={{padding: '32px 40px', maxWidth: '100%'}}>
    <div className="floor-detail-header">
      <h3>도서 검색</h3>
    ...
    <div style={{display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 24}}>
      {BOOK_CATEGORIES.map(c => (<button key={c} onClick={() => {
        setQuery('');
    ...
    {results === null && !loading && (
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 0}}>
          {quickCards.map(card => (<div
              key={card.key}
              onClick={card.onClick}
              style={{
                background: '#F7FAFC',
                border: `2px solid ${quickMode === card.key ? '#2C5282' : '#E2E8F0'}`,
                borderRadius: 10,
                padding: '22px 16px',
                textAlign: 'center',
                cursor: 'pointer',
    ...
    {/* 검색 결과 */}
    {!loading && results !== null && !error && (<div className="room-category">
      <div className="room-category-title"
           style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
            <span>
              {quickMode === 'top5' ? '🔥 인기 도서 Top 5' : quickMode === 'new' ? '📦 신규 입수 도서' : quickMode === 'return' ? '📅 반납 예정 도서' : '검색 결과'}
              <span className="room-category-count">{results.length}건</span>
            </span>
        <button
            style={{
              fontSize: '.78rem', color: '#A0AEC0', background: 'none', border: 'none', cursor: 'pointer',
            }}
            onClick={() => {
              setResults(null);
              setQuickMode(null);
      ...
      {results.length === 0 ? <div style={{padding: '30px 0', color: '#A0AEC0', textAlign: 'center'}}>
        {quickMode === 'return' ? '반납 예정 도서가 없습니다.' : '검색 결과가 없습니다.'}
      </div> : results.map(b => (<BookRow key={b.id} b={b}/>))}
    </div>)}
  </div>);
}
...
function StudySection({onNeedLogin}) {
  ...
  async function handleTimeClick(t, occupied) {
    ...
    const msg = `📍 ${room?.name}  (${room?.floor} · 최대 ${room?.capacity}인)\n` + `🛠 시설: ${room?.amenities?.join(', ')}\n\n` + `🗓 ${today}  ${t} ~ ${endTime} (1시간)\n\n` + `위 일정으로 예약하시겠습니까?`;
  ...
  return (<div className="caf-foodcourt">
    ...
    <div className="floor-detail">
      ...
      {selected && done && (<div style={{padding: '60px 40px', textAlign: 'center'}}>
  ...
  return (<div className="caf-foodcourt">
    <div className="floor-list">
      <div style={{padding: '16px 20px', background: '#F7FAFC', borderBottom: '1px solid #edf2f7'}}>
        <div style={{
          fontSize: '.72rem', color: '#A0AEC0', fontWeight: 700, letterSpacing: '.5px', marginBottom: 4,
        }}>전체 혼잡도
        </div>
      ...
      {data.floors.map(f => (<div key={f.name} className="floor-item" style={{cursor: 'default'}}>
        <div style={{flex: 1}}>
    ...
    <div className="floor-detail">
      ...
      <div className="room-category">
        ...
        <div style={{display: 'flex', alignItems: 'flex-end', gap: 4, height: 150, marginBottom: 8}}>
          {data.hourlyTrend.map(({hour, rate}) => {
            ...
            return (<div key={hour}
              ...
              <div style={{
                fontSize: '.58rem', color: isCurrent ? '#2C5282' : '#A0AEC0', fontWeight: isCurrent ? 700 : 400,
              }}>
                {hour}
              </div>
            </div>);
        ...
        <div style={{
          marginTop: 16, padding: '14px 16px', background: '#F7FAFC', borderRadius: 8, border: '1px solid #e2e8f0',
        }}>
  ...
  return (<div className="caf-foodcourt">
    <div className="floor-list">
      {notices.map(n => {
        ...
        return (<div key={n.id} className={`floor-item${isActive ? ' active' : ''}`} onClick={() => handleSelect(n)}>
          <div style={{flex: 1}}>
    ...
    <div className="floor-detail">
      ...
      {selectedId && !detailLoading && detail && (<>
        ...
        <div className="room-category">
          <div style={{
            borderTop: '1px solid #e2e8f0', paddingTop: 16, fontSize: '.9rem', color: '#4A5568', lineHeight: 1.9,
          }}>
            {detail.content}
          </div>
        </div>
...
export function LibraryPage({building}) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [rooms, studyRooms] = await Promise.all([fetchReadingRooms(), fetchStudyRooms()]);
  ...
  return (<>
    <div id="libraryView">
          ...
          <div className="info-grid">
            ...
            <div className="info-col">
              <div className="caf-today-summary">
                ...
                }].map(item => (<div key={item.label} className="caf-summary-row">
      ...
      <div className="building-tabs">
        {NAV_TABS.map(tab => (<button
            key={tab.key}
            className={activeTab === tab.key ? 'active' : ''}
            onClick={() => {
              if (activeTab === tab.key) setTabKey(k => k + 1); else setActiveTab(tab.key);
            }}
        >
          {tab.label}
        </button>))}
