...
import {
  fetchBuildings,
  fetchReadingRooms, fetchReadingRoomSeats, reserveSeat,
  searchBooks,
  fetchStudyRooms, fetchStudyRoomSlots, reserveStudyRoom,
  fetchCongestion,
  fetchNotices, fetchNotice,
} from '../data/api.js';
...
function LoadingBox() {
  return (
      <div style={{ padding: '40px', color: '#a0aec0', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 20, height: 20, border: '2px solid #e2e8f0', borderTopColor: '#2c5282', borderRadius: '50%', animation: 'lib-spin .7s linear infinite' }} />
        로딩 중...
      </div>
  );
}
function ErrorBox({ msg, onRetry }) {
  return (
      <div style={{ margin: 20, padding: '16px 20px', background: '#fff5f5', border: '1px solid #fed7d7', borderRadius: 8, color: '#c53030', fontSize: '.88rem', display: 'flex', alignItems: 'center', gap: 12 }}>
        ⚠️ {msg}
  ...
  return (
      <div className="room-row" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '.78rem', color: '#718096', marginTop: 2 }}>{b.author} · {b.publisher}{b.category ? ` · ${b.category}` : ''}</div>
        </div>
...
function ReadingSection({ buildingId, onNeedLogin }) {
  ...
  async function loadSeats(id, roomList) {
    setSeatLoading(true); setSeatData(null); setLocalReserved({});
    try { setSeatData(await fetchReadingRoomSeats(buildingId, id)); }
              ...
              <div className="room-category">
                ...
                <div style={{ display: 'flex', gap: 14, marginBottom: 12 }}>
                  {[['AVAILABLE','이용 가능','#38a169'],['OCCUPIED','사용 중','#e53e3e'],['RESERVED','예약됨','#d69e2e']].map(([k,v,c]) => (
                      <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '.73rem', color: '#718096' }}>
                  ...
                  {seats.map(s => {
                    ...
                    return (
                        <div
                            ...
                            onMouseEnter={e => { if (clickable) { e.currentTarget.style.transform = 'scale(1.15)'; e.currentTarget.style.boxShadow = `0 0 0 2px ${c}70`; e.currentTarget.style.zIndex = 2; } }}
...
function BooksSection({ buildingId, onNeedLogin }) {
  ...
  async function handleSearch(q = query, { publisher = '', category = '' } = {}) {
    setLoading(true); setError(null); setQuickMode('search'); setResults(null);
    try { setResults(await searchBooks(buildingId, { q, publisher, category })); }
    catch (e) { setError(e.message); }
    finally { setLoading(false); }
  }

  async function handleQuickNew() {
    setQuickMode('new'); setResults(null); setLoading(true); setError(null);
    try { setResults(await searchBooks(buildingId, { q: '' })); }
    catch (e) { setError(e.message); }
    finally { setLoading(false); }
  }
  ...
  return (
      <div className="floor-detail" style={{ padding: '32px 40px', maxWidth: '100%' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
          ...
          />
          <button className="btn btn-primary" onClick={() => handleSearch()} style={{ borderRadius: 8, whiteSpace: 'nowrap' }}>
            검색
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 24 }}>
          {BOOK_CATEGORIES.map(c => (
              <button key={c} onClick={() => { setQuery(''); handleSearch('', { category: c }); }}
                      style={{ padding: '4px 12px', border: '1px solid #e2e8f0', borderRadius: 20, background: '#f7fafc', color: '#718096', fontSize: '.78rem', cursor: 'pointer', fontFamily: 'Noto Sans KR, sans-serif' }}>
                {c}
              </button>
          ))}
        </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 0 }}>
              {quickCards.map(card => (
                  <div
                      ...
                      onMouseEnter={e => { e.currentTarget.style.background = '#ebf8ff'; e.currentTarget.style.borderColor = '#4299e1'; e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 6px 20px rgba(44,82,130,.15)'; }}
                      onMouseLeave={e => { e.currentTarget.style.background = '#f7fafc'; e.currentTarget.style.borderColor = quickMode === card.key ? '#2c5282' : '#e2e8f0'; e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = ''; }}
                  >
        ...
        {!loading && results !== null && !error && (
            <div className="room-category">
              <div className="room-category-title" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span>
              ...
              <span className="room-category-count">{results.length}건</span>
                ...
                <button
                    style={{ fontSize: '.78rem', color: '#a0aec0', background: 'none', border: 'none', cursor: 'pointer' }}
                    onClick={() => { setResults(null); setQuickMode(null); setQuery(''); }}
                >
                  ✕ 닫기
                </button>
              </div>
...
function StudySection({ buildingId, onNeedLogin }) {
  ...
  const load = useCallback(async () => {
    setLoading(true); setError(null);
    try { setRooms(await fetchStudyRooms(buildingId)); }
    catch (e) { setError(e.message); }
    finally { setLoading(false); }
  }, [buildingId]);
  ...
  async function handleSelectRoom(room) {
    ...
    setSelectedId(room.id); setSelectedTime(null); setSlotData(null);
    setSlotLoading(true); setDone(false); setLocalOccupied([]);
    try { setSlotData(await fetchStudyRoomSlots(buildingId, room.id, today)); }
    catch { setSlotData({ occupiedSlots: [] }); }
          ...
          {rooms.map(r => {
            ...
            return (
                <div
                    ...
                    className={`floor-item${isActive ? ' active' : ''}`}
                    onClick={() => handleSelectRoom(r)}
                    style={{ opacity: r.status !== 'AVAILABLE' ? 0.55 : 1, cursor: r.status !== 'AVAILABLE' ? 'not-allowed' : 'pointer' }}
                >
        ...
        <div className="floor-detail">
          ...
          {selected && done && (
              <div style={{ padding: '60px 40px', textAlign: 'center' }}>
                ...
                <div style={{ color: '#718096', fontSize: '.9rem' }}>{selected.name} · {today} · {selectedTime} (1시간)</div>
                <div style={{ color: '#a0aec0', fontSize: '.78rem', marginTop: 4 }}>내 예약에서 확인하실 수 있습니다.</div>
                <button className="btn btn-outline-secondary mt-4"
                        onClick={() => { setDone(false); setSelectedId(null); setSelectedTime(null); }}>
                  새 예약하기
                </button>
              </div>
          )}
...
function CongestionSection({ buildingId }) {
  ...
  const load = useCallback(async () => {
    setLoading(true); setError(null);
    try { setData(await fetchCongestion(buildingId)); }
    catch (e) { setError(e.message); }
    finally { setLoading(false); }
  }, [buildingId]);
      ...
      <div className="caf-foodcourt">
        <div className="floor-list">
          <div style={{ padding: '16px 20px', background: '#f7fafc', borderBottom: '1px solid #edf2f7' }}>
            <div style={{ fontSize: '.72rem', color: '#a0aec0', fontWeight: 700, letterSpacing: '.5px', marginBottom: 4 }}>전체 혼잡도</div>
              ...
              <div key={f.name} className="floor-item" style={{ cursor: 'default' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ marginTop: 5, background: '#edf2f7', borderRadius: 4, height: 4, overflow: 'hidden' }}>
                    <div style={{ width: `${f.rate}%`, background: congestionColor(f.rate), height: '100%', borderRadius: 4, transition: 'width .6s' }} />
                  </div>
                </div>
                <div className="floor-pin" style={{
        ...
        <div className="floor-detail">
          ...
          <div className="room-category">
              ...
              {data.hourlyTrend.map(({ hour, rate }) => {
                ...
                return (
                    <div key={hour} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                      ...
                      <div style={{ fontSize: '.58rem', color: isCurrent ? '#2c5282' : '#a0aec0', fontWeight: isCurrent ? 700 : 400 }}>
                        {hour}
                      </div>
                    </div>
                );
            ...
            <div style={{ marginTop: 16, padding: '14px 16px', background: '#f7fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}>
              <div style={{ fontSize: '.75rem', color: '#a0aec0' }}>💡 추천 방문 시간</div>
              <div style={{ fontWeight: 600, fontSize: '.9rem', color: '#2d3748', marginTop: 3 }}>오전 9시 이전 또는 오후 7시 이후</div>
            </div>
          </div>
        </div>
      </div>
...
function NoticesSection({ buildingId }) {
  ...
  async function handleSelect(notice) {
    setSelectedId(notice.id); setDetailLoading(true); setDetail(null);
    try { setDetail(await fetchNotice(buildingId, notice.id)); }
    catch { setDetail(notice); }
      ...
      <div className="caf-foodcourt">
        ...
        <div className="floor-detail">
          ...
          {selectedId && !detailLoading && detail && (<>
            ...
            <div className="room-category">
              <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: 16, fontSize: '.9rem', color: '#4a5568', lineHeight: 1.9 }}>
                {detail.content}
              </div>
            </div>
          </>)}
...
export function LibraryPage({ building }) {
  ...
  const [bookResetKey, setBookResetKey] = useState(0); // 도서 검색 탭 클릭 시 상태 초기화용
  ...
  function renderTab() {
    switch (activeTab) {
      case 'reading':    return <ReadingSection    buildingId={buildingId} onNeedLogin={handleNeedLogin} />;
      case 'books':      return <BooksSection      key={bookResetKey} buildingId={buildingId} onNeedLogin={handleNeedLogin} />;
      case 'study':      return <StudySection      buildingId={buildingId} onNeedLogin={handleNeedLogin} />;
      case 'congestion': return <CongestionSection buildingId={buildingId} />;
      case 'notices':    return <NoticesSection    buildingId={buildingId} />;
      default:           return null;
    }
  }
          ...
          <div className="building-tabs">
            {NAV_TABS.map(tab => (
                <button
                    ...
                    onClick={() => { setActiveTab(tab.key); if (tab.key === 'books') setBookResetKey(k => k + 1); }}
                >
                  {tab.label}
                </button>
            ))}
