...
public interface LibraryStudyRoomReservationRepository extends JpaRepository<LibraryStudyRoomReservation, Long> {
  // 특정 방 + 날짜 + 시간 중복 확인
  ...
  List<LibraryStudyRoomReservation> findByRoomIdAndDate(Long roomId, LocalDate date);

  // 유저의 전체 예약 내역 조회 (마이페이지용)
  List<LibraryStudyRoomReservation> findByUserNumber(String userNumber);

  // 유저의 예약 목록 조회 (내 예약 확인용)
  List<LibraryStudyRoomReservation> findByUserNumberAndDate(String userNumber, LocalDate date);
}
